...
class PipelineConfig:
    ...
    # Root of the Depth-Anything-3 project (where the scripts live).
    PROJECT_ROOT: Path = Path(__file__).resolve().parent

    # Python interpreter used to call sub-scripts (same venv).
    PYTHON: str = sys.executable

    # -- Pipeline script paths (relative to PROJECT_ROOT) --
    ...
    JOBS_ROOT: Path = PROJECT_ROOT / "_jobs"
...
def _prepare_job_environment(job_dir: Path, input_path: Path) -> None:
    ...
    Creates the folder structure the scripts expect:
        ...
        <job>/src  -> PROJECT_ROOT/src        (symlink to model code)
    ...
    for pkg_name in ("src", "depth_anything_3"):
        pkg = CFG.PROJECT_ROOT / pkg_name
        link = job_dir / pkg_name
        if pkg.exists() and not link.exists():
            link.symlink_to(pkg.resolve())

    # -- Symlink each pipeline script --
    for script in (CFG.STEP1_SCRIPT, CFG.STEP2_SCRIPT, CFG.STEP3_SCRIPT):
